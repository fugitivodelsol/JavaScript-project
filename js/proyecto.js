...
function agregarEventosEnMenu() {
  document
    .querySelector("#btnIniciarSesionContraseñaCreada")
    .addEventListener("click", menuBienvenidaHandler);
  document
    .querySelector("#btnCerrarSesionUsuario")
    .addEventListener("click", menuBienvenidaHandler);
  document
    .querySelector("#btnCerrarSesionLocales")
    .addEventListener("click", menuBienvenidaHandler);
  document
    .querySelector("#btnCrearContraseñaVolver")
    .addEventListener("click", menuBienvenidaHandler);
  document
    .querySelector("#btnIniciarSesionPerilCreado")
    .addEventListener("click", menuBienvenidaHandler);
  // por ahora para que ni ingrese. document.querySelector("#btnLogInTiendas").addEventListener("click", menuLocalesHandler);
  document
    .querySelector("#btncambiarContraseñaUsuario")
    .addEventListener("click", menuCambioDeContraseñaHandler);
  document
    .querySelector("#btnCrearPerfil")
    .addEventListener("click", menuCrearPerfilHandler);
  document
    .querySelector("#btnCrearPerfil")
    .addEventListener("click", menuRegistroUsuarioHandler);
  document
    .querySelector("#btnVovlerDsdRererva")
    .addEventListener("click", menuUsuariosHandler);
}
//////////////////////////////////////////////////////////////////////////////
...
function ocultarPantallas() {
  ...
  document.querySelector("#idDivTablaReservasFinalizadas").style.display =
    "none";
  document.querySelector("#idListadoDeReservasPendientes").style.display =
    "none";
...
function funcionEventoBotones() {
  ...
  document
    .querySelector("#btnReservaUsu1")
    .addEventListener("click", hacerReserva);
  document
    .querySelector("#btListaReservasPendientes")
    .addEventListener("click", verListaDeReservasPendientes);
  document
    .querySelector("#btnVerDatosEstadisticosUsuario")
    .addEventListener("click", tablaEstadisticaUsuario);
  document
    .querySelector("#btnVerLocalPreferido")
    .addEventListener("click", localPreferido);
  document
    .querySelector("#btnCalificarReservasFinalizadasUsuario")
    .addEventListener("click", verListaDeReservas);
  document
    .querySelector("#btnBuscarReservasPorNobre")
    .addEventListener("click", buscar);
  document
    .querySelector("#btnEditarCupoMax")
    .addEventListener("click", agregarCapacidadManual);
  document
    .querySelector("#btListaReservasPendientesLocal")
    .addEventListener("click", listaDeReservas);
  document
    .querySelector("#btnPorcentajeOcupacionLocal")
    .addEventListener("click", porcentajeDeOcupacionDelLocal);
  document
    .querySelector("#btnReservasPendientesTotal")
    .addEventListener("click", mostrarReservasPendientes);
  document
    .querySelector("#btnReservasFinalizadas")
    .addEventListener("click", mostrarReservasFinalizadas);
}
//Evento de boton para validar contraseña
function agregarEventosBtnCrearContraseña() {
  document
    .querySelector("#btnCambiarContraseña")
    .addEventListener("click", cambiarContraseña);
  document
    .querySelector("#btnIniciarSesion")
    .addEventListener("click", botonIngresarUsuario); // iniciar sesion como cliente
  document
    .querySelector("#btnLogInTiendas")
    .addEventListener("click", botonIngresarLocal); //iniciar sesion como tienda
}
...
function cambiarContraseña() {
  ...
  if (resultado == true) {
    ...
    mensaje =
      ...
      " " +
      "La contraseña deberá contener por lo menos: 6 dígitos, una letra mayúscula, una letra mayúscula, una minúscula, un número";
...
function botonIngresarLocal() {
  ...
  if (encontroLocal != null) {
    let contraseñaCorrectaLocal =
      encontroLocal.contraseñaLocal == contraseñaIngresada;
...
function botonIngresarUsuario() {
  ...
  if (encontroCliente != null) {
    let contraseñaCorrectaUsuario =
      encontroCliente.contraseñaUsuario == contraseñaIngresada;
...
function registro() {
  ...
  if (encontroCliente || encontroLocal) {
    ...
    ingresarClientes(usuarioIngresado, contraseñaIngresada);
...
function capacidadDisponibleLocal(array, nomLocal, capacidadLocal) {
  ...
  while (i < array.length && tieneCapacidad == false) {
    let localActual = array[i];
    if (
      localActual.nombreLocal == nomLocal &&
...
function hacerReserva() {
  ...
  let cantidadPersonas = parseInt(
    document.querySelector("#inputCantidadDePersonas").value
  );
  let estadoDeReserva = buscarEstadoReserva(
    reservasArray,
    nombreCliente,
    nombreLocal
  );
  let reservaRepetida = buscarReservaDeUsuario(
    reservasArray,
    nombreCliente,
    nombreLocal
  );
  let tieneCapacidad = capacidadDisponibleLocal(
    localesArray,
    nombreLocal,
    cantidadPersonas
  );
...
function buscarReservaDeUsuario(array, nombreUsu, nombreLoc) {
  ...
  while (resultado == null && i < array.length) {
    let usuarioActual = array[i];
    if (
      usuarioActual.nombreCliente == nombreUsu &&
      usuarioActual.local == nombreLoc &&
...
function buscarReservaPendientePorNombre(array, nombreUsu) {
  //Cambia el estado de pendiente a finalizado
  let resultado = null;
  let i = 0;
  while (resultado == null && i < array.length) {
    let usuarioActual = array[i];
...
function verListaDeReservas() {
  ...
  for (let i = 0; i < reservasArray.length; i++) {
    ...
    if (
      nombreCliente == reservaActual.nombreCliente &&
      reservaActual.estado == "finalizado"
    ) {
      bodyHTML += `
            <tr>
                <td>
...
function btnAgregarCalificacionEnArray() {
  ...
  for (let i = 0; i < reservasArray.length; i++) {
    ...
    if (
      reservaAct.nombreCliente == nombreCliente &&
      reservaAct.local == nombreLocal
    ) {
...
function cancelarReserva() {
  ...
  for (let i = 0; i < reservasArray.length; i++) {
    ...
    if (
      reservaDeUsuario.idReserva == idRes &&
...
function porcentaje(ocupado, total) {
...
function cantidadDeReservasFinUsuPorLocal(array, usu, local) {
  ///Usar para local preferido
  let contador = 0;
  for (let i = 0; i < array.length; i++) {
    let reservaAct = array[i];
    if (
      reservaAct.nombreCliente == usu &&
      reservaAct.local == local &&
...
function tablaEstadisticaUsuario() {
  ...
  for (let i = 0; i < localesArray.length; i++) {
    ...
    let cantidadDeReservasFin = cantidadDeReservasFinUsuPorLocal(
      reservasArray,
      nombreCliente,
      localActual
    ); //cantidad de reservas finalizadas por el usuario en este local.
    let totalResPorLocal = totalReservasPorLocal(
      reservasArray,
      reservaActual.nombreLocal
    ); //Cantidad total de reservas de un local independientemente de su estado.
    let porsentajeDelUsuEnLocal = porcentaje(
      cantidadDeReservasFin,
      totalResPorLocal
    ); //Porcentaje del usuario en el total de las reservas que tiene el local independientemente de su estado.
  ...
  document.querySelector("#bodyTablaDatosEstadisticosUsuario").innerHTML =
    bodyHTML;
}
...
function localPreferido() {
  ...
  for (let i = 0; i < localesArray.length; i++) {
    ...
    for (let j = 0; j < reservasArray.length; j++) {
      let reservaActual = reservasArray[i];
      if (
        localActual.nombreLocal == reservaActual.local &&
        nombreCliente == reservaActual.nombreCliente &&
        reservaActual.estado == "finalizado"
      ) {
        contador++;
  ...
  if (contador == 0) {
    mensaje = "Usted no teien reservas finalizadas";
  }
...
function buscar() {
  ...
  for (let i = 0; i < reservasArray.length; i++) {
    ...
    if (
      esSubcadena == true &&
      arrayAct.estado == "pendiente" &&
      arrayAct.local == nombreLocal
    ) {
      contador++;
      bodyHTML += `
                    <tr>
                     ...
                     `;
      document.querySelector("#bodyTablaReservasPendientesLocales").innerHTML =
        bodyHTML;
    }
  }
...
function agregarCapacidadManual() {
  let capacidadAgregada = parseInt(
    document.querySelector("#inputEditarCupoMax").value
  );
  ...
  document.querySelector("#parrafoEditarCupoMax").innerHTML =
    "Su capacidad fue cargada con éxito" +
    " la capacidad total es de : " +
    capacidadTotal;
}
...
function completarListadoDeReservas() {
  ...
  let arrayDeBotones = document.querySelectorAll(
    ".BotonCambiarEstadoDeReserva"
  );
  for (let i = 0; i < arrayDeBotones.length; i++) {
    let botonActual = arrayDeBotones[i];
...
function cambiarEstadoDeReserva(nombreCliente) {
  let reservaDeUsuario = buscarReservaPendientePorNombre(
    reservasArray,
    nombreCliente
  );
...
function porcentajeDeOcupacionDelLocal() {
  ...
  let cantidadTotalReservasPendiente = cantidadReservaPendiente(
    reservasArray,
    nombreLocal
  );
  let porcentajeDeOcupacionDelLocal = porcentaje(
    cantidadTotalReservasPendiente,
    totalCapaidadDelLocal
  );
  document.querySelector("#numTotalDeReservasPendientes").innerHTML =
    "El porcentaje de ocupación es de: " + porcentajeDeOcupacionDelLocal + "%";
}
...
function mostrarReservasPendientes() {
  let nombreLocal = document.querySelector("#inputUserName").value;
  let totalReservasPendientes = cantidadReservaPendientes(
    reservasArray,
    nombreLocal
  );
  document.querySelector("#mensajeMostrarReservasPendientes").innerHTML =
    "el total de reservas pendientes es de: " + totalReservasPendientes;
}
function mostrarReservasFinalizadas() {
  let nombreLocal = document.querySelector("#inputUserName").value;
  let totalReservasFinalizadas = cantidadReservaFinalizada(
    reservasArray,
    nombreLocal
  );
  document.querySelector("#mensajeMostrarReservasFinalizadas").innerHTML =
    "el total de reservas finalizadas es de: " + totalReservasFinalizadas;
}
function cantidadReservaFinalizada(array, nombreLoc) {
  ...
  while (i < array.length) {
    ...
    if (
      usuarioActual.local == nombreLoc &&
      usuarioActual.estado == "finalizado"
    ) {
      cantidadFinalida++;
    }
    i++;
